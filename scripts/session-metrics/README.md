...
## Scripts
  ...
  | `tools.py`          | Per-tool call volume and failure rates, failure taxonomy, Edit/Write retry recovery, durations/output sizes, per-turn parallelism, Bash `reason` coverage per model                          |
  ...
  | `time_breakdown.py` | Invocation wall time alongside API elapsed time, cumulative tool work, and scheduled retry delays; tool/model totals, turn pacing, think time between tasks, slowest operations              |
...
- `time_breakdown.py` cannot derive an exclusive wall-time remainder from current telemetry. Tool-call durations are cumulative work and may overlap, while retry delays are scheduled values rather than observed waits. Incomplete invocations have no `session_summary` wall time and are excluded from wall-time totals.
