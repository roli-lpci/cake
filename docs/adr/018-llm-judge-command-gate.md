...
Verdict parsing consumes the complete response as JSON, apart from surrounding whitespace and one optional Markdown code block with an empty or `json` language tag. Text outside that block, trailing data after the JSON object, multiple values or blocks, and invalid JSON (including unescaped control characters in strings) are malformed verdicts. Tool-argument JSON repair is a separate compatibility path and must not repair or truncate judge responses. This clarifies the strict, fail-closed contract in response to #470.
